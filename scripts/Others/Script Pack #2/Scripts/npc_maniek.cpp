...
public:
    ...
    bool OnGossipHello(Player *pPlayer, Creature *pCreature)
    {
        ...
        pPlayer->ADD_GOSSIP_ITEM( 10, "　传送：　", GOSSIP_SENDER_MAIN, 6111);

        if (ALLIANCE == pPlayer->GetTeam())
        {
            pPlayer->ADD_GOSSIP_ITEM( 5, "　暴风城　", GOSSIP_SENDER_MAIN, 6201);
            pPlayer->ADD_GOSSIP_ITEM( 5, "　铁炉堡　", GOSSIP_SENDER_MAIN, 6202);
            pPlayer->ADD_GOSSIP_ITEM( 5, "　达纳苏斯　", GOSSIP_SENDER_MAIN, 6203);
            pPlayer->ADD_GOSSIP_ITEM( 5, "　埃索达　", GOSSIP_SENDER_MAIN, 6204);
            if (PlayerLevel>=68)
                pPlayer->ADD_GOSSIP_ITEM( 5, "　诺森德　", GOSSIP_SENDER_MAIN, 6267);
        }
        else
        {
            pPlayer->ADD_GOSSIP_ITEM( 5, "　奥格瑞玛　", GOSSIP_SENDER_MAIN, 6211);
            pPlayer->ADD_GOSSIP_ITEM( 5, "　雷霆崖　", GOSSIP_SENDER_MAIN, 6212);
            pPlayer->ADD_GOSSIP_ITEM( 5, "　幽暗城　", GOSSIP_SENDER_MAIN, 6213);
            pPlayer->ADD_GOSSIP_ITEM( 5, "　银月城　", GOSSIP_SENDER_MAIN, 6214);
            if (PlayerLevel>=68)
                pPlayer->ADD_GOSSIP_ITEM( 5, "　诺森德　", GOSSIP_SENDER_MAIN, 6268);
        }

        if (pPlayer->getClass()==6)
        {
            pPlayer->ADD_GOSSIP_ITEM( 5, "　阿彻鲁斯黑锋要塞　", GOSSIP_SENDER_MAIN, 6207);
        }

        pPlayer->ADD_GOSSIP_ITEM( 5, "　阿斯特兰纳　", GOSSIP_SENDER_MAIN, 6266);

        if (PlayerLevel>=30)
            pPlayer->ADD_GOSSIP_ITEM( 5, "　古拉巴什竞技场　", GOSSIP_SENDER_MAIN, 6200);
            pPlayer->ADD_GOSSIP_ITEM( 5, "　加基森　", GOSSIP_SENDER_MAIN, 6269);

        if (PlayerLevel>=58)
            pPlayer->ADD_GOSSIP_ITEM( 5, "　沙塔斯　", GOSSIP_SENDER_MAIN, 6208);

        if (PlayerLevel>=65)
            pPlayer->ADD_GOSSIP_ITEM( 5, "　奎尔丹纳斯岛　", GOSSIP_SENDER_MAIN, 6209);

        if (PlayerLevel>=70)
            pPlayer->ADD_GOSSIP_ITEM( 5, "　达拉然　", GOSSIP_SENDER_MAIN, 6210);
