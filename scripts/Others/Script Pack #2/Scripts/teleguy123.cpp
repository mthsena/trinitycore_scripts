...
class teleguy : public CreatureScript
{
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        pPlayer->ADD_GOSSIP_ITEM(5, "|cffff0000[Дуель Зона]|r"               , GOSSIP_SENDER_MAIN, 1200);
        pPlayer->ADD_GOSSIP_ITEM(5, "|cffff0000[Арена Гурубаши]|r"                   , GOSSIP_SENDER_MAIN, 1291);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        switch(action)
        {
            ...
            player->CLOSE_GOSSIP_MENU();
               ...
               ChatHandler(player).PSendSysMessage("[ПВП Система]: Вы вступили в бой!");
               
               char MSG[200];
               sprintf(MSG, "[ПВП Система]: Игрок '|cff0000f1 %s |r' вступил в бой!", player->GetName());
               sWorld->SendZoneText(pvp_zone_id, MSG);
             }
           }else
              ChatHandler(player).PSendSysMessage("Слишком много игроков, подождите пока освободиться место!");
              break;
