...
class multi_transmog_season : public CreatureScript
{
	public:
		multi_transmog_season() : CreatureScript("multi_transmog_season")
		{
		}
			...
			switch(uiAction)
			{
				...
				case 4:
					if (Player->GetArenaPersonalRating(ARENA_TYPE_2v2) >= arenarating4 || 
						Player->GetArenaPersonalRating(ARENA_TYPE_3v3) >= arenarating4 ||
						Player->GetArenaPersonalRating(ARENA_TYPE_5v5) >= arenarating4)
					{
...
void AddSC_multi_transmog_season()
{
	new multi_transmog_season();
}